...
public class Utils {
    ...
    public static File getArtifactFromConfiguration(SpringCloudAppConfig springConfiguration) throws MojoExecutionException {
        final SpringCloudAppDeploymentConfig deploymentConfig = springConfiguration.getDeployment();
        final List<File> files = getArtifacts(deploymentConfig.getResources());
        return getRunnableArtifactFromFiles(files);
    }
